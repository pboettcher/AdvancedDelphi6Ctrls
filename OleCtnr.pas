...
type
  ...
  public
    ...
    property ChangeDecimalSymbol:boolean read FChangeDecimalSymbol write FChangeDecimalSymbol; //Временное решение!!!
    property CanPaste: Boolean read GetCanPaste;
    property ForcedDecimalSymbol:Char read FForcedDecimalSymbol write FForcedDecimalSymbol; //Временное решение!!!
  ...
  {Реализован новый интерфейс: ICustomOleForm}
...
{Получить десятичный разделитель}
...
begin
  ...
  {Сменить разделитель на запятую на время открытия MSWord (а лучше бы
   найти способ выводить диалоговое окно MSWord на поверхность)}
  DS:=',';
  ...
  if FChangeDecimalSymbol then
    if DS<>FForcedDecimalSymbol then SetDecimSep(DS); //Вернуть разделитель на место
end;
...
begin
  FSaveWidth:=FForm.ClientWidth+1; //Принуждаем выполнить ResizeBorder
  OnResize;
end;
